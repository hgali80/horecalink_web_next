...
export async function listErpCashMovements(maxRows = 24) {
  ...
  return snap.docs.map((item) => {
    ...
    return {
      ...
      status: text(data.status || "posted"),
      cancellationReason: text(data.cancellationReason),
      cancelledByName: text(data.cancelledByName || data.cancelledBy),
      cancelledDateLabel: formatDate(data.cancelledAt),
      documentId: text(data.documentId),
      advanceFromCancellation: data.advanceFromCancellation === true,
      originalDocumentNo: text(data.originalDocumentNo),
