...
import { listProductPresentations } from "@/app/satissitok/services/productPresentationService";
    ...
    <div className="mx-auto max-w-7xl space-y-6 p-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-slate-900">Ürün Fiyat Sunumları</h1>
            <p className="text-sm text-slate-500">Fotoğraf, ürün bilgileri, birim ve birim fiyattan oluşan sade sunumlar.</p>
          </div>
        </div>
            ...
            <Link key={item.id} href={`/satissitok/admin/product-presentations/${item.id}`} className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm transition hover:border-[#1d3246]">
              <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                <div>
                  ...
                  <div className="mt-1 text-sm text-slate-500">{item.items?.length || 0} ürün · {formatDate(item.updatedAt || item.createdAt)}</div>
                </div>
                <span className={`w-fit rounded-full px-3 py-1 text-xs font-bold ${item.status === "ready" ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"}`}>
