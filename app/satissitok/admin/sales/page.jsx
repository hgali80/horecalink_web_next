...
export default function SalesListPage() {
  ...
  const goHref = (r) => (r.status === "draft" || r.status === "pending" ? `/satissitok/admin/sales/new?draftId=${r.id}` : `/satissitok/admin/sales/${r.id}`);
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 md:p-8 space-y-6 bg-gray-50/50 min-h-screen">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div><h1 className="text-2xl font-bold text-gray-900 tracking-tight">Satış Yönetimi</h1></div>
        <Link href="/satissitok/admin/sales/new" className="inline-flex items-center justify-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-2.5 rounded-xl font-semibold shadow-sm"><Plus size={18} /><span>Yeni Satış Oluştur</span></Link>
      </div>
      <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex flex-wrap gap-4 items-center">
        ...
        <select className="bg-gray-50 ring-1 ring-gray-200 rounded-lg py-1.5 px-3 text-sm" value={saleType} onChange={(e) => setSaleType(e.target.value)}><option value="">Tüm Türler</option><option value="official">Resmi Satış</option><option value="actual">Fiili Satış</option></select>
        <select className="bg-gray-50 ring-1 ring-gray-200 rounded-lg py-1.5 px-3 text-sm" value={platformId} onChange={(e) => setPlatformId(e.target.value)}><option value="">Tüm Platformlar</option><option value="kaspi">Kaspi</option><option value="ozon">Ozon</option><option value="showroom">Showroom</option><option value="online">Online</option></select>
            ...
            <tbody className="bg-white divide-y divide-gray-100">
              {rows.map((r) => (
                <tr key={r.id} className={`group transition-colors ${r.status === "draft" ? "bg-amber-50/60" : r.status === "pending" ? "bg-blue-50/50" : r.status === "cancelled" ? "bg-gray-50/80" : "hover:bg-indigo-50/30"}`}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap"><div className="flex flex-col gap-1"><span className="text-sm text-gray-700">{r.saleType === "official" ? "🏢 Resmi" : "📦 Fiili"}</span><span className="text-xs text-gray-400 font-medium italic">@{r.saleChannel}</span></div></td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap"><div className="flex items-center gap-2 text-sm text-gray-500 font-medium"><Calendar size={14} className="text-gray-400" />{formatDate(r.invoiceDate || r.documentDate)}</div></td>
