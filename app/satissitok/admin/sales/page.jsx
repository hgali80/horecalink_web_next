...
export default function SalesListPage() {
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      try {
        let qy = query(collection(db, "sales"), orderBy("createdAt", "desc"));
        if (saleType) qy = query(qy, where("saleType", "==", saleType));
        if (platformId) qy = query(qy, where("saleChannel", "==", platformId));
        if (status) qy = query(qy, where("status", "==", status));

        const saleSnap = await getDocs(qy);
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((r) => {
      const cari = (caris[r.cariId] || "").toLowerCase();
      const invoice = (r.invoiceNo || r.saleNo || "").toLowerCase();
      const platform = (r.saleChannel || "").toLowerCase();
      return cari.includes(needle) || invoice.includes(needle) || platform.includes(needle);
    });
  }, [rows, q, caris]);
    ...
    <div className="max-w-7xl mx-auto p-4 md:p-8 space-y-6 bg-gray-50/50 min-h-screen">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500">Taslak ve tamamlanmış satış faturalarını buradan yönet.</p>
        </div>
        <Link
      ...
      <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex flex-wrap gap-4 items-center">
        ...
        <div className="flex flex-col gap-1 min-w-[240px]">
          <label className="text-[10px] font-bold text-gray-400 uppercase ml-1">Ara</label>
          <div className="flex items-center gap-2 bg-gray-50 ring-1 ring-gray-200 rounded-lg px-3 py-1.5">
            <Search size={16} className="text-gray-400" />
            <input
              className="bg-transparent outline-none text-sm w-full"
              placeholder="Fatura, cari veya platform..."
              value={q}
        ...
        <div className="flex flex-col gap-1">
          ...
          <select className="bg-gray-50 ring-1 ring-gray-200 rounded-lg py-1.5 px-3 text-sm outline-none" value={saleType} onChange={(e) => setSaleType(e.target.value)}>
        ...
        <div className="flex flex-col gap-1">
          ...
          <select className="bg-gray-50 ring-1 ring-gray-200 rounded-lg py-1.5 px-3 text-sm outline-none" value={platformId} onChange={(e) => setPlatformId(e.target.value)}>
        ...
        <div className="flex flex-col gap-1">
          ...
          <select className="bg-gray-50 ring-1 ring-gray-200 rounded-lg py-1.5 px-3 text-sm outline-none" value={status} onChange={(e) => setStatus(e.target.value)}>
                  ...
                  <tr key={r.id} className={`group hover:bg-indigo-50/30 transition-colors ${isCancelled ? "bg-gray-50/80" : ""}`}>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center gap-3">
                        <div className={`p-2 rounded-lg ${isDraft ? "bg-amber-50 text-amber-600" : isCancelled ? "bg-gray-200" : "bg-indigo-50 text-indigo-600"}`}>
                          <CreditCard size={18} />
                        </div>
                        <div>
                          <Link href={href} className={`block font-semibold hover:text-indigo-600 transition-colors ${isCancelled ? "text-gray-400 line-through" : "text-gray-900"}`}>
                            {r.invoiceNo || r.saleNo || (isDraft ? "Kaydedilmiş Taslak" : "N/A")}
                          </Link>
                          {isDraft ? (
                            ...
                            <span className="text-[11px] text-gray-400 font-medium tracking-tight">Final Kayıt</span>
                          )}
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm font-semibold text-gray-800">{caris[r.cariId] || r.cariId || "—"}</div>
                    </td>

                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center gap-2 text-sm text-gray-500 font-medium">
                        <Calendar size={14} className="text-gray-400" />
                        {fmtDate(r.invoiceDate || r.documentDate || r.updatedAt || r.createdAt)}
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right">
                      <div className={`text-base font-bold ${isCancelled ? "text-gray-400 line-through" : "text-gray-900"}`}>
                        ...
                        <span className="text-[10px] ml-1 text-gray-400 font-normal underline decoration-indigo-200">KZT</span>
                      </div>
                    </td>

                    <td className="px-6 py-4 whitespace-nowrap text-center">
                      <div className="flex items-center justify-center gap-2">
                        {r.hasNegativeStock && !isDraft && (
                        ...
                        <Link href={href} className="p-1.5 hover:bg-white rounded-full transition-shadow hover:shadow-sm text-gray-400 hover:text-indigo-600">
                ...
                <tr>
                  <td colSpan={6} className="px-6 py-12 text-center text-gray-400">
                    <div className="flex flex-col items-center gap-2 text-gray-300">
                      ...
                      <p className="text-sm font-medium italic">Aradığın kritere uygun satış yok.</p>
                    </div>
                  </td>
                </tr>
              )}
