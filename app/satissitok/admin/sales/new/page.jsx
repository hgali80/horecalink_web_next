...
import { finalizeSale, getSaleDraft, saveSaleDraft } from "@/app/satissitok/services/saleService";
  ...
  async function loadAll() {
    ...
    try {
      const [p, c, b, s, draft] = await Promise.all([
        ...
        loadBalances(),
        getSettings(),
        draftId ? getSaleDraft(draftId) : Promise.resolve(null),
      ]);
      setProducts(p);
      setCaris(c);
      setBalances(b);
  ...
  async function handleSubmit(payload) {
    ...
    try {
      if (payload?.status === "draft") {
        const res = await saveSaleDraft({ ...payload, draftId }, draftId || null);
        const nextDraftId = res?.saleId;
        if (!nextDraftId) throw new Error("Taslak kaydedildi fakat draftId alınamadı.");
        router.replace(`/satissitok/admin/sales/new?draftId=${nextDraftId}`);
        alert("Satış taslağı Firestore'a kaydedildi.");
        setDraftDoc(await getSaleDraft(nextDraftId));
        return;
      }
    ...
    <main className="w-full px-4 py-6 space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200 pb-6">
        ...
        <div className="flex items-center space-x-2 text-xs">
          <span className="px-3 py-1 bg-blue-50 text-blue-700 rounded-full font-medium border border-blue-100">Fatura Modülü</span>
          {draftDoc?.draftNo && <span className="px-3 py-1 bg-amber-50 text-amber-700 rounded-full font-medium border border-amber-100">{draftDoc.draftNo}</span>}
          {saving && (
        ...
        <div className="relative bg-white rounded-xl shadow-sm border border-slate-200 overflow-visible">
          <div className="p-1">
            <SaleForm
              ...
              draftMeta={draftDoc}
              isEditingDraft={Boolean(draftId)}
            />
          </div>
        </div>
      </div>
      ...
      <div className="rounded-lg bg-slate-50 p-4 border border-slate-200">
        <p className="text-xs text-slate-500 leading-relaxed italic">
          * Taslak kaydında gerçek fatura numarası tüketilmez. Final kayıt anında sistem otomatik numara üretir.
        </p>
      </div>
    </main>
  );
