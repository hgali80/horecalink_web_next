...
import { summarizeAllCariBalances } from "@/app/satissitok/admin/cari/services/cariTransactions";
...
export async function loadFinanceDashboard({
  ...
  returnsSnap.forEach((d) => {
    const s = d.data();
    if (String(s.status || "").toLowerCase() !== "returned") return;
    const type = s.saleType === "actual" ? "actual" : "official";
  ...
  expensesSnap.forEach((d) => {
    const t = d.data();
    if (String(t.txType || "").toLowerCase() !== "expense") return;
  ...
  otherIncomeSnap.forEach((d) => {
    const t = d.data();
    if (String(t.txType || "").toLowerCase() !== "other_income") return;
    const net = round2(t.amountNet ?? t.amount ?? 0);
    const vat = round2(t.vatAmount ?? 0);
  ...
  Object.values(cariBalanceMap || {}).forEach((entry) => {
