...
function getBrandLabel(product) {
  return String(product?.brand || "Diğer").trim() || "Diğer";
}
...
export default function CatalogListingClient({
  ...
  const tree = useMemo(() => buildCatalogTree(), []);
  const labels = useMemo(
    () => getCatalogLabels({ group, category, subcategory }),
  ...
  const availableBrands = useMemo(() => {
    return [...new Set(products.map(getBrandLabel))].sort((a, b) =>
  ...
  }, [products, brandValue, searchValue, sortValue]);
  ...
  const pageDescription = subcategory
    ? "Seçili alt kategoriye ait aktif ve yayında olan ürünler listelenir."
    : category
    ? "Seçili kategoriye ait tüm aktif ve yayında olan ürünler listelenir."
    : "Bu ürün grubuna ait tüm aktif ve yayında olan ürünler listelenir. Sol menüden kategori ve alt kategori filtreleyebilirsiniz.";
                ...
                <div className="mt-4 space-y-3">
                  {groupItem.categories.map((categoryItem) => {
                    const categorySelected =
                      groupItem.key === group && categoryItem.key === category;

                    const categoryHref = getCategoryHref(
                      groupItem.key,
                      categoryItem.key
                    );
      ...
      <div className="rounded-[28px] bg-[#f2f4f6] p-6">
        ...
        <div className="mt-4 space-y-2">
          ...
          >
            <span>Все</span>
            {!brandValue ? (
              <CheckCircle2 className="h-4 w-4 text-[#1d3246]" />
            ) : null}
          </button>
    ...
    <section className="bg-[#f7f8fa] pb-20 pt-10">
      <div className="mx-auto w-full max-w-[1600px] px-4 md:px-6 xl:px-8">
        <div className="mb-6 flex items-center gap-2 text-[13px] text-slate-500">
          ...
          {category ? (
            <>
              ...
              {subcategory ? (
                <Link
                  href={`/catalog/${group}/${category}`}
                  className="hover:text-[#1d3246]"
                >
                  {labels.categoryLabel}
                </Link>
              ) : (
          ...
          <div className="mt-6 grid gap-4 lg:grid-cols-[minmax(0,1fr)_240px_180px]">
            <div className="relative">
              ...
              <input
                ...
                placeholder="Ürün, marka, kod ara..."
            ...
            >
              <option value="sort_order">Varsayılan sıralama</option>
        ...
        <div className="grid gap-8 lg:grid-cols-[320px_minmax(0,1fr)] xl:grid-cols-[340px_minmax(0,1fr)]">
          ...
          <div>
            {mobileFilterOpen ? (
              <div className="mb-6 lg:hidden">{sidebar}</div>
            ) : null}
            ...
            {loading ? (
              ...
              <div className="rounded-[28px] bg-white px-8 py-16 text-center shadow-[0_18px_60px_rgba(29,50,70,0.06)]">
                ...
                <p className="mt-3 text-[15px] leading-7 text-slate-500">
                  Bu kategori için yayınlanmış ürün yok ya da filtre sonucu boş.
                </p>
              </div>
            )}
          </div>
